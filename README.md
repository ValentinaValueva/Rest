...
Запускайте всю последовательность одним запуском программы. Серверная сессия ограничена по времени, поэтому GET, POST, PUT и DELETE должны выполняться подряд с одним и тем же `JSESSIONID`.
